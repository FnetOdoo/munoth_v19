...
class StockProductionLOt(models.Model):
    ...
    anode_electrode_id = fields.Many2one('anode.electrode.making')
...
class Utilities(models.Model):
    ...
    anode_electrode_id = fields.Many2one('anode.electrode.making')
    anode_electrode_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_anode_electrode_utility_condition')

    def _compute_anode_electrode_utility_condition(self):
        for rec in self:
            rec.anode_electrode_utility_condition = 'fail'
            if rec.anode_electrode_id:
                if rec.anode_electrode_id.operation_id.min_humidity <= rec.humidity <= rec.anode_electrode_id.operation_id.max_humidity and rec.anode_electrode_id.operation_id.min_temperature <= rec.temperature <= rec.anode_electrode_id.operation_id.max_temperature:
                    rec.anode_electrode_utility_condition = 'good'
                else:
                    rec.anode_electrode_utility_condition = 'fail'
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_anode_electrode_product_id(self):
        if self.anode_electrode_id:
            self.location_src_id = self.anode_electrode_id.location_src_id.id
            self.location_dest_id = self.anode_electrode_id.location_dest_id.id


class AnodeElectrodeMaking(models.Model):
    _name = 'anode.electrode.making'
    _description = 'Anode Electrode Making'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(AnodeElectrodeMaking, self).default_get(fields)
        # operation = self.env['manufacturing.operation'].search([('type', '=', 'anode_electrode_making')], limit=1)
    ...
    ], default='anode_electrode_making')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'anode_electrode_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'anode_electrode_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'anode_electrode_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'anode_electrode_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    anode_drying_id = fields.Many2one('anode.drying')
    production_plan_id = fields.Many2one('production.plan')
    anode_drying_count = fields.Integer(compute='_compute_anode_drying_count')
    breakdown_ids = fields.One2many('production.breakdown', 'anode_electrode_id')
    ...
    def _compute_anode_drying_count(self):
        for rec in self:
            rec.anode_drying_count = self.env['anode.drying'].search_count([('id', '=', rec.anode_drying_id.id)])

    def show_anode_drying_record(self):
        records = self.env['anode.drying'].search([('id', '=', self.anode_drying_id.id)])
        if len(records) == 1:
            return {
                'name': _('Anode Drying'),
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'anode.drying',
                'domain': [('id', '=', self.anode_drying_id.id)]
            }
    ...
    #             [('type', '=', 'anode_electrode_making'), ('product_model_id', '=', self.product_model_id.id)], limit=1)
    #         if operation:
    ...
    def action_confirm(self):
        ...
        for rec in self.component_ids:
            ...
            picking = self.env['stock.picking'].create({
                ...
                'location_dest_id': rec.anode_electrode_id.production_location_id.id,
            ...
            stock_move = self.env['stock.move'].create({
                ...
                'company_id': rec.env.company.id,
                'anode_electrode_id': rec.anode_electrode_id.id,
    ...
    def action_view_product_move(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'name': _("Stock Move"),
            'domain': [('anode_electrode_id', '=', self.id)],
            'view_mode': 'list,form',
        }
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') or vals['name'] == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('manufacturing.anode.electrode.making') or _('New')
        return super(AnodeElectrodeMaking, self).create(vals_list)
        ...
        for rec in self:
            ...
            return {
                ...
                'context': {
                    ...
                    'default_anode_electrode_id': rec.id,
                },
            }
        ...
        for rec in self:
            if rec.anode_electrode_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'company_id': rec.env.company.id,
                    'move_type': 'direct',
                    'origin': rec.anode_electrode_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.anode_electrode_id.name,
                    'product_id': rec.product_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.anode_electrode_id:
                if rec.type == 'HOLD':
                    rec.anode_electrode_id.state = 'hold'
                else:
                    rec.anode_electrode_id.state = 'progress'
