...
class MachineData(models.Model):
    ...
    capacity_test_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    capacity_id = fields.Many2one('capacity.test')
    capacity_tester_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_capacity_test_cell_utility')

    def _compute_capacity_test_cell_utility(self):
        for rec in self:
            rec.capacity_tester_utility_condition = 'fail'
            if rec.capacity_id:
                if rec.capacity_id.operation_id.min_humidity <= rec.humidity <= rec.capacity_id.operation_id.max_humidity and rec.capacity_id.operation_id.min_temperature <= rec.temperature <= rec.capacity_id.operation_id.max_temperature:
                    rec.capacity_tester_utility_condition = 'good'
                else:
                    rec.capacity_tester_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    capacity_id = fields.Many2one('capacity.test')
    out_capacity_id = fields.Many2one('capacity.test')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_capacity_test_product_id(self):
        if self.capacity_id:
            self.location_src_id = self.capacity_id.location_src_id.id
            self.location_dest_id = self.capacity_id.location_dest_id.id
...
class CapacityTest(models.Model):
    ...
    def default_get(self, fields):
        defaults = super(CapacityTest, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'capacity_id', string='Serial Number')
    input_material_lines = fields.One2many('material.line', 'capacity_id')
    name = fields.Char(
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'capacity_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'capacity_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'capacity_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'capacity_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    machine_id = fields.Many2one('manufacturing.machine')
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    product_tray_id = fields.Many2one('product.tray')
    breakdown_ids = fields.One2many('production.breakdown', 'capacity_id')
    ...
    finished_move_ids = fields.One2many('stock.move.line', 'out_capacity_id')
    ...
    fai_status_capacity = fields.Boolean(compute='_compute_fai_status_capacity')
    process_status_capacity = fields.Boolean(compute='_compute_process_status_capacity')
    ...
    def _compute_packing_count(self):
        for rec in self:
            rec.packing_count = self.env['package.move'].search_count([('capacity_id', '=', rec.id)])
    ...
    def _compute_fai_status_capacity(self):
        for rec in self:
            fai = self.env['first.article.inspection'].search([('origin', '=', rec.name), ('state', '=', 'done')])
            if fai:
                rec.fai_status_capacity = True
            else:
                rec.fai_status_capacity = False


    @api.depends('pa_count')
    def _compute_process_status_capacity(self):
        for rec in self:
            process = self.env['first.article.inspection'].search(
                [('origin', '=', rec.name), ('state', '=', 'done'), ('inspection_type', '=', 'process')])
            if process:
                rec.process_status_capacity = True
            else:
                rec.process_status_capacity = False
    ...
    def _compute_pad_print_count(self):
        for rec in self:
            rec.pad_print_count = self.env['pad.printing'].search_count([('id', '=', rec.pad_printing_id.id)])

    def _compute_voltage_test_count(self):
        for rec in self:
            rec.voltage_test_count = self.env['voltage.test'].search_count([('capacity_id', '=', rec.id)])

    def action_pad_printing_record(self):
        records = self.env['pad.printing'].search([('id', '=', self.pad_printing_id.id)])
        if len(records) == 1:
            return {
                'name': _('Pad Print'),
                'type': 'ir.actions.act_window',
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'pad.printing',
                'domain': [('id', '=', self.pad_printing_id.id)]
            }

    def show_voltage_test_record(self):
        records = self.env['voltage.test'].search([('capacity_id', '=', self.id)])
        if len(records) == 1:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code('capacity.test') or _('New')
    ...
    def show_package_record(self):
        records = self.env['package.move'].search([('capacity_id', '=', self.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
                'name': _('Package'),
        ...
        for rec in self:
            if rec.capacity_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.capacity_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'name': rec.capacity_id.name,
                    'product_id': rec.product_id.id,
                    ...
                    'company_id': rec.env.company.id,
                    'capacity_id': rec.capacity_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.capacity_id:
                if rec.type == 'HOLD':
                    rec.capacity_id.state = 'hold'
                else:
                    rec.capacity_id.state = 'progress'
