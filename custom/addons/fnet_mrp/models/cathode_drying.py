...
class MachineData(models.Model):
    ...
    cathode_drying_id = fields.Many2one('cathode.drying')
    cathode_dry_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_cathode_dry_condition')

    def _compute_cathode_dry_condition(self):
        for rec in self:
            rec.cathode_dry_condition = 'fail'
            if rec.cathode_dry_condition:
                if rec.cathode_drying_id.operation_id.dry_min_temperature <= rec.temperature <= rec.cathode_drying_id.operation_id.dry_max_temperature and abs(
                        rec.cathode_drying_id.operation_id.dry_min_vacuum) <= abs(rec.vacuum) <= abs(
                        rec.cathode_drying_id.operation_id.dry_max_vacuum):
                    rec.cathode_dry_condition = 'good'
                else:
                    rec.cathode_dry_condition = 'fail'
...
class Utilities(models.Model):
    ...
    cathode_drying_id = fields.Many2one('cathode.drying')
    cathode_drying_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_cathode_drying_utility_condition')

    def _compute_cathode_drying_utility_condition(self):
        for rec in self:
            rec.cathode_drying_utility_condition = 'fail'
            if rec.cathode_drying_id:
                if rec.cathode_drying_id.operation_id.min_humidity <= rec.humidity <= rec.cathode_drying_id.operation_id.max_humidity and rec.cathode_drying_id.operation_id.min_temperature <= rec.temperature <= rec.cathode_drying_id.operation_id.max_temperature:
                    rec.cathode_drying_utility_condition = 'good'
                else:
                    rec.cathode_drying_utility_condition = 'fail'
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_cathode_drying_product_id(self):
        if self.cathode_drying_id:
            self.location_src_id = self.cathode_drying_id.location_src_id.id
            self.location_dest_id = self.cathode_drying_id.location_dest_id.id


class CathodeDrying(models.Model):
    _name = 'cathode.drying'
    ...
    def default_get(self, fields):
        defaults = super(CathodeDrying, self).default_get(fields)
        production_location = self.env['stock.location'].search([('usage', '=', 'production')], limit=1)
        # operation = self.env['manufacturing.operation'].search([('type', '=', 'cathode_drying')], limit=1)
    ...
    type = fields.Selection([
        ...
        ('cathode_drying', 'Anode Drying'),
        ('cathode_drying', 'Cathode Drying'),
        ('diaphragm_drying', 'Diaphragm Drying'),
        ('cathode_electrode_making', 'Anode Electrode Making'),
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'cathode_drying_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'cathode_drying_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'cathode_drying_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'cathode_drying_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    cathode_electrode_count = fields.Integer(compute='_compute_cathode_electrode_count')
    cathode_slitting_count = fields.Integer(compute='_compute_cathode_slitting_count')
    breakdown_ids = fields.One2many('production.breakdown', 'cathode_drying_id')
    ...
    def _compute_cathode_electrode_count(self):
        for rec in self:
            rec.cathode_electrode_count = self.env['cathode.electrode.making'].search_count(
                [('cathode_drying_id', '=', rec.id)])

    def _compute_cathode_slitting_count(self):
        for rec in self:
            rec.cathode_slitting_count = self.env['cathode.slitting'].search_count([('id', '=', rec.cathode_slitting_id.id)])

    def show_cathode_electrode_record(self):
        records = self.env['cathode.electrode.making'].search([('cathode_drying_id', '=', self.id)])
        if len(records) == 1:
            return {
                'name': _('Cathode Electrode Making'),
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'cathode.electrode.making',
                'domain': [('cathode_drying_id', '=', self.id)]
            }

    def show_cathode_slitting_record(self):
        records = self.env['cathode.slitting'].search([('id', '=', self.cathode_slitting_id.id)])
        if len(records) == 1:
            return {
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'cathode.slitting',
                'domain': [('id', '=', self.cathode_slitting_id.id)]
            }

    def action_create_cathode_electrode_making(self):
        cathode_electrode_making = self.env['cathode.electrode.making'].create({
            'cathode_drying_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id
        })
        cathode_electrode_making._onchange_of_product_plan_id()
        return {
            'name': _('Cathode Electrode Making'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'cathode.electrode.making',
            'res_id': cathode_electrode_making.id
        }
    ...
    def _onchange_of_product_plan_id(self):
        if self.production_plan_id:
            production_operation = self.env['production.operation'].search(
                [('operation_type', '=', 'cathode_drying'), ('production_plan_id', '=', self.production_plan_id.id)],
                limit=1)
    ...
    #             'location_dest_id': rec.cathode_drying_id.production_location_id.id,
    #             'company_id': rec.env.company.id,
    #
    #             'move_type': 'direct',
    #             'origin': self.name,
    ...
    #             'company_id': rec.env.company.id,
    #             'cathode_drying_id': rec.cathode_drying_id.id,
        ...
        for rec in self:
            ...
            return {
                ...
                'context': {
                    ...
                    'default_cathode_drying_id': rec.id,
                },
            }
        ...
        for rec in self:
            if rec.cathode_drying_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'company_id': rec.env.company.id,
                    'move_type': 'direct',
                    'origin': rec.cathode_drying_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'name': rec.cathode_drying_id.name,
                    'product_id': rec.product_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.cathode_drying_id:
                if rec.type == 'HOLD':
                    rec.cathode_drying_id.state = 'hold'
                else:
                    rec.cathode_drying_id.state = 'progress'
