...
class StockProductionLOt(models.Model):
    ...
    cathode_electrode_id = fields.Many2one('cathode.electrode.making')
...
class Utilities(models.Model):
    ...
    cathode_electrode_id = fields.Many2one('cathode.electrode.making')
    cathode_electrode_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_cathode_electrode_utility_condition')

    def _compute_cathode_electrode_utility_condition(self):
        for rec in self:
            rec.cathode_electrode_utility_condition = 'fail'
            if rec.cathode_electrode_id:
                if rec.cathode_electrode_id.operation_id.min_humidity <= rec.humidity <= rec.cathode_electrode_id.operation_id.max_humidity and rec.cathode_electrode_id.operation_id.min_temperature <= rec.temperature <= rec.cathode_electrode_id.operation_id.max_temperature:
                    rec.cathode_electrode_utility_condition = 'good'
                else:
                    rec.cathode_electrode_utility_condition = 'fail'
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_cathode_electrode_product_id(self):
        if self.cathode_electrode_id:
            self.location_src_id = self.cathode_electrode_id.location_src_id.id
            self.location_dest_id = self.cathode_electrode_id.location_dest_id.id


class CathodeElectrodeMaking(models.Model):
    _name = 'cathode.electrode.making'
    _description = 'Cathode Electrode Making'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(CathodeElectrodeMaking, self).default_get(fields)
    ...
    ], default='cathode_electrode_making')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'cathode_electrode_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'cathode_electrode_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'cathode_electrode_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'cathode_electrode_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    cathode_drying_id = fields.Many2one('cathode.drying')
    cathode_drying_count = fields.Integer(compute='_compute_cathode_drying_count')
    breakdown_ids = fields.One2many('production.breakdown', 'cathode_electrode_id')
    ...
    def _compute_cathode_drying_count(self):
        for rec in self:
            rec.cathode_drying_count = self.env['cathode.drying'].search_count([('id', '=', rec.cathode_drying_id.id)])

    def show_cathode_drying_record(self):
        records = self.env['cathode.drying'].search([('id', '=', self.cathode_drying_id.id)])
        if len(records) == 1:
            return {
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'cathode.drying',
                'domain': [('id', '=', self.cathode_drying_id.id)]
            }
    ...
    #             [('type', '=', 'cathode_electrode_making'), ('product_model_id', '=', self.product_model_id.id)], limit=1)
    #         if operation:
    ...
    def action_confirm(self):
        ...
        for rec in self.component_ids:
            ...
            picking = self.env['stock.picking'].create({
                ...
                'location_dest_id': rec.cathode_electrode_id.production_location_id.id,
            ...
            stock_move = self.env['stock.move'].create({
                ...
                'company_id': rec.env.company.id,
                'cathode_electrode_id': rec.cathode_electrode_id.id,
    ...
    def action_view_product_move(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'name': _("Stock Move"),
            'domain': [('cathode_electrode_id', '=', self.id)],
            'view_mode': 'list,form',
        }
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'manufacturing.cathode.electrode.making'
                ) or _('New')
        ...
        for rec in self:
            ...
            return {
                ...
                'context': {
                    ...
                    'default_cathode_electrode_id': rec.id,
                },
            }
        ...
        for rec in self:
            if rec.cathode_electrode_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'company_id': rec.env.company.id,
                    'move_type': 'direct',
                    'origin': rec.cathode_electrode_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.cathode_electrode_id.name,
                    'product_id': rec.product_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.cathode_electrode_id:
                if rec.type == 'HOLD':
                    rec.cathode_electrode_id.state = 'hold'
                else:
                    rec.cathode_electrode_id.state = 'progress'
