...
class Utilities(models.Model):
    ...
    cathode_slitting_id = fields.Many2one('cathode.slitting')
    cathode_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_cathode_slitting_utility_condition')

    def _compute_cathode_slitting_utility_condition(self):
        for rec in self:
            rec.cathode_utility_condition = 'fail'
            if rec.cathode_slitting_id:
                if rec.cathode_slitting_id.operation_id.min_humidity <= rec.humidity <= rec.cathode_slitting_id.operation_id.max_humidity and rec.cathode_slitting_id.operation_id.min_temperature <= rec.temperature <= rec.cathode_slitting_id.operation_id.max_temperature:
                    rec.cathode_utility_condition = 'good'
                else:
                    rec.cathode_utility_condition = 'fail'
...
class AmbientCondition(models.Model):
    ...
    cathode_slitting_id = fields.Many2one('cathode.slitting')
    cathode_ambient_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute = '_compute_cathode_ambient_condition')

    def _compute_cathode_ambient_condition(self):
        for rec in self:
            rec.ambient_condition = 'fail'
            if rec.cathode_ambient_condition:
                if rec.cathode_slitting_id.operation_id.min_humidity <= rec.humidity <= rec.cathode_slitting_id.operation_id.max_humidity and rec.cathode_slitting_id.operation_id.min_temperature <= rec.temperature <= rec.cathode_slitting_id.operation_id.max_temperature:
                    rec.cathode_ambient_condition = 'good'
                else:
                    rec.cathode_ambient_condition ='fail'
            else:
                rec.cathode_ambient_condition = 'fail'
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_cathode_slitting_product_id(self):
        if self.cathode_slitting_id:
            self.location_src_id = self.cathode_slitting_id.location_src_id.id
            self.location_dest_id = self.cathode_slitting_id.location_dest_id.id



class CathodeSlittingRolls(models.Model):
    _name = 'cathode.slitting'
    _description = 'Cathode Slitting Rolls'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(CathodeSlittingRolls, self).default_get(fields)
        production_location = self.env['stock.location'].search([('usage', '=', 'production')], limit=1)
        # operation = self.env['manufacturing.operation'].search([('type', '=', 'cathode_slitting')], limit=1)
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'cathode_slitting_id', 'Components',
        copy=False)
    ...
    ambient_condition_ids = fields.One2many(
        'ambient.condition', 'cathode_slitting_id', 'Ambient Condition',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})

    quality_ids = fields.One2many(
        'quality.parameter', 'cathode_slitting_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})

    equipment_ids = fields.One2many(
        'equipment.details', 'cathode_slitting_id', 'Equipment',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    machine_data_ids = fields.One2many(
        'machine.parameter', 'cathode_slitting_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'cathode_slitting_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    cathode_drying_id = fields.Many2one('cathode.drying')
    production_plan_id = fields.Many2one('production.plan')
    cathode_drying_count = fields.Integer(compute='_compute_drying_count')
    breakdown_ids = fields.One2many('production.breakdown', 'cathode_slitting_id')
    ...
    def _compute_drying_count(self):
        for rec in self:
            rec.cathode_drying_count = self.env['cathode.drying'].search_count([('cathode_slitting_id', '=', rec.id)])

    def action_create_cathode_drying(self):
        cathode_drying = self.env['cathode.drying'].create({
            'cathode_slitting_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id
        })
        cathode_drying._onchange_of_product_plan_id()
        return {
            'name': _('Cathode Drying'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'cathode.drying',
            'res_id': cathode_drying.id
        }

    def show_cathode_drying_record(self):
        records = self.env['cathode.drying'].search([('cathode_slitting_id', '=', self.id)])
        if len(records) == 1:
    ...
    def _onchange_product_qty(self):
        if self.product_qty:
            for line in self.component_ids:
                ...
                if bom_line:
                    qty = (bom_line[0].product_qty / self.bom_id.product_qty) * self.product_qty
    ...
    # def action_start(self):
    #     for rec in self:
    #         # stock_move = self.env['stock.move'].search([('anode_slitting_id', '=', rec.id)])
    #         # stock_move._action_done()
    #         rec._check_availability()
    #         for line in rec.component_ids:
    #             stock_move = self.env['stock.move'].create({
        ...
        for rec in self:
            ...
            return {
                ...
                'context': {
                    ...
                    'default_cathode_slitting_id': rec.id,
        ...
        for rec in self:
            if rec.cathode_slitting_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'company_id': rec.env.company.id,
                    'move_type': 'direct',
                    'origin': rec.cathode_slitting_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'name': rec.cathode_slitting_id.name,
                    'product_id': rec.product_id.id,
                    ...
                    'company_id': rec.env.company.id,
                    'cathode_slitting_id': rec.cathode_slitting_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.cathode_slitting_id:
                if rec.type == 'HOLD':
                    rec.cathode_slitting_id.state = 'hold'
                else:
                    rec.cathode_slitting_id.state = 'progress'
