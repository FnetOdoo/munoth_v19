...
class MachineData(models.Model):
    ...
    clamp_baking_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    clamp_baking_id = fields.Many2one('cell.clamp.baking')
    clamp_baking_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_cb_cell_utility')

    def _compute_cb_cell_utility(self):
        for rec in self:
            rec.clamp_baking_utility_condition = 'fail'
            if rec.clamp_baking_id:
                if rec.clamp_baking_id.operation_id.min_humidity <= rec.humidity <= rec.clamp_baking_id.operation_id.max_humidity and rec.clamp_baking_id.operation_id.min_temperature <= rec.temperature <= rec.clamp_baking_id.operation_id.max_temperature:
                    rec.clamp_baking_utility_condition = 'good'
                else:
                    rec.clamp_baking_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    clamp_baking_id = fields.Many2one('cell.clamp.baking')
    out_clamp_baking_id = fields.Many2one('cell.clamp.baking')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_clamp_baking_product_id(self):
        if self.clamp_baking_id:
            self.location_src_id = self.clamp_baking_id.location_src_id.id
            self.location_dest_id = self.clamp_baking_id.location_dest_id.id
...
class ClampBaking(models.Model):
    ...
    def default_get(self, fields):
        defaults = super(ClampBaking, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'clamp_baking_id', string='Serial Number')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'clamp_baking_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'clamp_baking_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'clamp_baking_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'clamp_baking_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    machine_id = fields.Many2one('manufacturing.machine')
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    product_tray_id = fields.Many2one('product.tray')
    breakdown_ids = fields.One2many('production.breakdown', 'clamp_baking_id')
    ht_cell_id = fields.Many2one('high.temperature.cell')
    cell_drying_id = fields.Many2one('cell.drying')
    ht_cell_count = fields.Integer(compute='_compute_ht_cell_count_count')
    aged_formation_count = fields.Integer(compute='_compute_aged_formation_count')
    finished_move_ids = fields.One2many('stock.move.line', 'out_clamp_baking_id')
    expected_end_time = fields.Datetime("Expected End", compute='compute_end_date')
    out_tray_id = fields.Many2one('product.tray')
    input_material_lines = fields.One2many('material.line', 'clamp_baking_id')
    remaining_hours = fields.Float("Remaining Time", compute='_get_remaining_time')
    fai_status_cbf = fields.Boolean(compute='_compute_fai_status_cbf')
    process_status_cbf = fields.Boolean(compute='_compute_process_status_cbf')
    ...
    def _compute_degas_count(self):
        for rec in self:
            rec.degas_count = self.env['degas.cell'].search_count([('clamp_baking_id', '=', rec.id)])

    def show_degas_record(self):
        records = self.env['degas.cell'].search([('clamp_baking_id', '=', self.id)])
    ...
    def _compute_ht_cell_count_count(self):
        for rec in self:
            rec.ht_cell_count = self.env['high.temperature.cell'].search_count([('id', '=', rec.ht_cell_id.id)])

    def _compute_aged_formation_count(self):
        for rec in self:
            rec.aged_formation_count = self.env['aged.formation.cell'].search_count([('clamp_baking_id', '=', rec.id)])

    def action_ht_cell_record(self):
        records = self.env['high.temperature.cell'].search([('id', '=', self.ht_cell_id.id)])
        if len(records) == 1:
            return {
                'name': _('Cell Injection'),
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'res_model': 'high.temperature.cell',
                'res_id': records.id,  # Assuming records is a single record
            }
        else:
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'high.temperature.cell',
                'domain': [('id', '=', self.ht_cell_id.id)]
            }

    def show_aged_formation_record(self):
        records = self.env['aged.formation.cell'].search([('clamp_baking_id', '=', self.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
    ...
    def action_create_aged_formation(self):
        aged_formation = self.env['aged.formation.cell'].create({
            'clamp_baking_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id
        })
        aged_formation._onchange_of_product_plan_id()
    ...
    def _onchange_of_product_plan_id(self):
        if self.production_plan_id:
            ...
            print("--------", production_operation,"------production_operation---\n")
    ...
    def action_done_production(self):
        for rec in self:
            ...
            if not self.component_ids:
                raise UserError(_("Please reserve material to confirm the production."))
            stock_moves = []
    ...
    def open_aged_formation_cell_processing(self):
        return {
            'name': _('RT Aging'),
            'type': 'ir.actions.act_window',
            'view_mode': 'list,form',
    ...
    def action_update_lot_product(self):
        for rec in self:
            return {
                ...
                'target': 'new',
                'context': {
                    'default_clamp_baking_id': rec.id,
        ...
        for rec in self:
            if rec.clamp_baking_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.clamp_baking_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.clamp_baking_id.name,
                    'product_id': rec.product_id.id,
                    ...
                    'company_id': rec.env.company.id,
                    'clamp_baking_id': rec.clamp_baking_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.clamp_baking_id:
                if rec.type == 'HOLD':
                    rec.clamp_baking_id.state = 'hold'
                else:
                    rec.clamp_baking_id.state = 'progress'
