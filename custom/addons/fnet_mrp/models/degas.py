...
class MachineData(models.Model):
    ...
    degas_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    degas_id = fields.Many2one('degas.cell')
    degas_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_degas_cell_utility')

    def _compute_degas_cell_utility(self):
        for rec in self:
            rec.degas_utility_condition = 'fail'
            if rec.degas_id:
                if rec.degas_id.operation_id.min_humidity <= rec.humidity <= rec.degas_id.operation_id.max_humidity and rec.degas_id.operation_id.min_temperature <= rec.temperature <= rec.degas_id.operation_id.max_temperature:
                    rec.degas_utility_condition = 'good'
                else:
                    rec.degas_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    degas_id = fields.Many2one('degas.cell')
    out_degas_id = fields.Many2one('degas.cell')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_degas_product_id(self):
        if self.degas_id:
            self.location_src_id = self.degas_id.location_src_id.id
            self.location_dest_id = self.degas_id.location_dest_id.id
...
class DegasCell(models.Model):
    _name = 'degas.cell'
    _description = 'Degas Cell'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(DegasCell, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'degas_id', string='Serial Number')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'degas_id', 'Components',
        copy=False)
    ...
    product_uom_id = fields.Many2one(
        'uom.uom', 'Product Unit of Measure', default=_get_default_product_uom_id, required=True,
    ...
    dsf_id = fields.Many2one('double.side.folding')
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'degas_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'degas_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'degas_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    resistance_ids=fields.One2many(
        'insulation.resistance.test','degas_id','Insulation Resistance',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    machine_id = fields.Many2one('manufacturing.machine')
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    product_tray_id = fields.Many2one('product.tray')
    breakdown_ids = fields.One2many('production.breakdown', 'degas_id')
    ...
    pad_print_count = fields.Integer(compute='_compute_pad_print_count')
    finished_move_ids = fields.One2many('stock.move.line', 'out_degas_id')
    expected_end_time = fields.Datetime("Expected End", compute='compute_end_date')
    out_tray_id = fields.Many2one('product.tray')
    input_material_lines = fields.One2many('material.line', 'degas_id')
    remaining_hours = fields.Float("Remaining Time", compute='_get_remaining_time')
    fai_status_degas = fields.Boolean(compute='_compute_fai_status_degas')
    process_status_degas = fields.Boolean(compute='_compute_process_status_degas')
    ...
    def _compute_aged_formation_count(self):
        for rec in self:
            rec.aged_formation_count = self.env['aged.formation.cell'].search_count([('id', '=', rec.aged_formation_id.id)])

    def _compute_pad_print_count(self):
        for rec in self:
            rec.pad_print_count = self.env['pad.printing'].search_count([('degas_id', '=', rec.id)])

    def action_aged_formation_record(self):
    ...
    def show_pad_printing_record(self):
        records = self.env['pad.printing'].search([('degas_id', '=', self.id)])
        if len(records) == 1:
    ...
    def action_create_pad_printing_cell(self):
        pad_printing = self.env['pad.printing'].create({
            'degas_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id,
            'product_qty': self.product_qty,
        })
        pad_printing._onchange_of_product_plan_id()
        return {
            'name': _('Pad Printing'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'pad.printing',
            'res_id': pad_printing.id
        }
    ...
    def open_double_side_folding_processing(self):
        return {
            'name': _('Double Side Folding'),
            'type': 'ir.actions.act_window',
            'view_mode': 'list,form',
            'res_model': 'double.side.folding',
            'domain': [('id', '=', self.dsf_id.id)]
        }
        ...
        for rec in self:
            if rec.degas_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.degas_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.degas_id.name,
                    'product_id': rec.product_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.degas_id:
                if rec.type == 'HOLD':
                    rec.degas_id.state = 'hold'
                else:
                    rec.degas_id.state = 'progress'
