...
class MachineData(models.Model):
    ...
    high_temperature_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    ht_cell_id = fields.Many2one('high.temperature.cell')
    ht_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_ht_cell_utility')

    def _compute_ht_cell_utility(self):
        for rec in self:
            rec.ht_utility_condition = 'fail'
            if rec.ht_cell_id:
                if rec.ht_cell_id.operation_id.min_humidity <= rec.humidity <= rec.ht_cell_id.operation_id.max_humidity and rec.ht_cell_id.operation_id.min_temperature <= rec.temperature <= rec.ht_cell_id.operation_id.max_temperature:
                    rec.ht_utility_condition = 'good'
                else:
                    rec.ht_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    ht_cell_id = fields.Many2one('high.temperature.cell')
    out_ht_cell_id = fields.Many2one('high.temperature.cell')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_ht_product_id(self):
        if self.ht_cell_id:
            self.location_src_id = self.ht_cell_id.location_src_id.id
            self.location_dest_id = self.ht_cell_id.location_dest_id.id
...
class HTCell(models.Model):
    _name = 'high.temperature.cell'
    _description = 'High Temperature Cell'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(HTCell, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'ht_cell_id', string='Serial Number')
    name = fields.Char(
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'ht_cell_id', 'Components',
        copy=False)
    ...
    formation_id = fields.Many2one('cell.clamp.baking')
    ...
    def _compute_cell_injection_count(self):
        for rec in self:
            rec.cell_injection_count = self.env['cell.injection'].search_count([('id', '=', rec.injection_id.id)])

    def _compute_clamp_baking_count(self):
        for rec in self:
            rec.clamp_baking_count = self.env['cell.clamp.baking'].search_count([('ht_cell_id', '=', rec.id)])
    ...
    def action_cell_injection_record_record(self):
        records = self.env['cell.injection'].search([('id', '=', self.injection_id.id)])
        if len(records) == 1:
            return {
    ...
    def show_clamp_baking_record(self):
        records = self.env['cell.clamp.baking'].search([('ht_cell_id', '=', self.id)])
        if len(records) == 1:
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'manufacturing.high.temperature.cell'
                ) or _('New')
        ...
        for rec in self:
            if rec.ht_cell_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.ht_cell_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.ht_cell_id.name,
                    'product_id': rec.product_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.ht_cell_id:
                if rec.type == 'HOLD':
                    rec.ht_cell_id.state = 'hold'
                else:
                    rec.ht_cell_id.state = 'progress'
