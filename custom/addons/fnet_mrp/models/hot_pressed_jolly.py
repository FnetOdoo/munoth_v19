...
class Utilities(models.Model):
    ...
    hot_press_id = fields.Many2one('hot.press.jelly')
    hot_press_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_hot_press_utility')

    def _compute_hot_press_utility(self):
        for rec in self:
            rec.hot_press_utility_condition = 'fail'
            if rec.hot_press_id:
                if rec.hot_press_id.operation_id.min_humidity <= rec.humidity <= rec.hot_press_id.operation_id.max_humidity and rec.hot_press_id.operation_id.min_temperature <= rec.temperature <= rec.hot_press_id.operation_id.max_temperature:
                    rec.hot_press_utility_condition = 'good'
                else:
                    rec.hot_press_utility_condition = 'fail'
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_hot_press_product_id(self):
        if self.hot_press_id:
            self.location_src_id = self.hot_press_id.location_src_id.id
            self.location_dest_id = self.hot_press_id.location_dest_id.id


class HotPressedJelly(models.Model):
    _name = 'hot.press.jelly'
    _description = 'Hot Press Jelly'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(HotPressedJelly, self).default_get(fields)
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'hot_press_id', 'Components',
        copy=False)
    ...
    breakdown_ids = fields.One2many('production.breakdown', 'hot_press_id')
    assembly_count = fields.Integer(compute='_compute_assembly_count')
    winding_count = fields.Integer(compute='_compute_winding_count')
    ...
    def _compute_assembly_count(self):
        for rec in self:
            rec.assembly_count = self.env['assembly.cell'].search_count([('hot_press_id', '=', rec.id)])

    def _compute_winding_count(self):
        for rec in self:
            rec.winding_count = self.env['winding'].search_count([('id', '=', rec.winding_id.id)])

    def action_show_winding_record(self):
        records = self.env['winding'].search([('id', '=', self.winding_id.id)])
        if len(records) == 1:
            return {
                'name': _('Winding'),
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'winding',
                'domain': [('id', '=', self.winding_id.id)]
            }

    def action_create_assembly_jelly(self):
        assembly_cell = self.env['assembly.cell'].create({
            'hot_press_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id
        })
        assembly_cell._onchange_of_product_plan_id()
        return {
            'name': _('Assembly Cell'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'assembly.cell',
            'res_id': assembly_cell.id
        }

    def show_assembly_cell_record(self):
        records = self.env['assembly.cell'].search([('hot_press_id', '=', self.id)])
        if len(records) == 1:
    ...
    #             [('type', '=', 'hot_press_jelly'), ('product_model_id', '=', self.product_model_id.id)], limit=1)
    #         if operation:
    ...
    def action_confirm(self):
        ...
        for rec in self.component_ids:
            ...
            picking = self.env['stock.picking'].create({
                ...
                'location_dest_id': rec.hot_press_id.production_location_id.id,
            ...
            stock_move = self.env['stock.move'].create({
                ...
                'company_id': rec.env.company.id,
                'hot_press_id': rec.hot_press_id.id,
    ...
    def open_assembly_processing(self):
        return {
        ...
        for rec in self:
            if rec.hot_press_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'company_id': rec.env.company.id,
                    'move_type': 'direct',
                    'origin': rec.hot_press_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.hot_press_id.name,
                    'product_id': rec.product_id.id,
...
class PressedJelly(models.Model):
    _name = 'pressed.jelly'
    _description = "Pressed Jelly"

class CellFormation(models.Model):
    _name = 'cell.formation'
    _description = "Cell Form"
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.hot_press_id:
                if rec.type == 'HOLD':
                    rec.hot_press_id.state = 'hold'
                else:
                    rec.hot_press_id.state = 'progress'
