...
class MachineData(models.Model):
    ...
    injection_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    injection_id = fields.Many2one('cell.injection')
    injection_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_injection_utility')

    def _compute_injection_utility(self):
        for rec in self:
            rec.injection_utility_condition = 'fail'
            if rec.injection_id:
                if rec.injection_id.operation_id.min_humidity <= rec.humidity <= rec.injection_id.operation_id.max_humidity and rec.injection_id.operation_id.min_temperature <= rec.temperature <= rec.injection_id.operation_id.max_temperature:
                    rec.injection_utility_condition = 'good'
                else:
                    rec.injection_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    injection_id = fields.Many2one('cell.injection')
    out_injection_id = fields.Many2one('cell.injection')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_product_injection_id(self):
        if self.injection_id:
            self.location_src_id = self.injection_id.location_src_id.id
            self.location_dest_id = self.injection_id.location_dest_id.id


class CellInjection(models.Model):
    _name = 'cell.injection'
    _description = 'Cell Injection'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _order = "id desc"

    @api.model
    def default_get(self, fields):
        defaults = super(CellInjection, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'injection_id', string='Serial Number')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'injection_id', 'Components',
        copy=False)
    ...
    high_temperature_id = fields.Many2one('high.temperature.cell')
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'injection_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'injection_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'injection_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    machine_id = fields.Many2one('manufacturing.machine')
    start_time = fields.Datetime()
    end_time = fields.Datetime()
    product_tray_id = fields.Many2one('product.tray')
    breakdown_ids = fields.One2many('production.breakdown', 'injection_id')
    cell_drying_id = fields.Many2one('cell.drying')
    cell_dry_count = fields.Integer(compute='_compute_cell_dry_count')
    ht_cell_count = fields.Integer(compute='_compute_ht_cell_count')
    finished_move_ids = fields.One2many('stock.move.line', 'out_injection_id')
    ...
    input_material_lines = fields.One2many('material.line', 'injection_id')
    remaining_hours = fields.Float("Remaining Time", compute='_get_remaining_time')
    first_inspection = fields.Many2one('first.article.inspection')
    fai_status = fields.Boolean(compute='_compute_fai_status')
    process_status = fields.Boolean(compute='_compute_process_status')
    ...
    def _compute_fai_status(self):
        for rec in self:
            ...
            if fai:
                rec.fai_status = True
        print("-----------", self.fai_status, "-----self.fai_status------\n")

    @api.depends('pa_count')
    def _compute_process_status(self):
        for rec in self:
            process = self.env['first.article.inspection'].search([('origin', '=', rec.name), ('state', '=', 'done'),('inspection_type','=','process')])
            if process:
                rec.process_status = True
            else:
                rec.process_status =False
    ...
    def _compute_cell_dry_count(self):
        for rec in self:
            rec.cell_dry_count = self.env['cell.drying'].search_count([('id', '=', rec.cell_drying_id.id)])

    def _compute_ht_cell_count(self):
        for rec in self:
            rec.ht_cell_count = self.env['high.temperature.cell'].search_count([('injection_id', '=', rec.id)])

    def action_cell_dry_record_record(self):
        records = self.env['cell.drying'].search([('id', '=', self.cell_drying_id.id)])
        if len(records) == 1:
            return {
                'name': _('Cell Drying'),
                'type': 'ir.actions.act_window',
    ...
    def show_ht_cell_record(self):
        records = self.env['high.temperature.cell'].search([('injection_id', '=', self.id)])
        if len(records) == 1:
    ...
    def action_create_ht_cell(self):
        ht_cell = self.env['high.temperature.cell'].create({
            'injection_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id,
            'product_qty': self.product_qty
        })
        ht_cell._onchange_of_product_plan_id()
        return {
            'name': _('HT Cell'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
    ...
    def action_done_production(self):
        ...
        if self.product_id.tracking != 'none' and len(self.lot_ids) != self.product_qty:
            raise UserError(_("Quantity to produce is %s but you have entered %s serial numbers." % (self.product_qty,
                                                                                                     len(self.lot_ids))))
    ...
    def open_high_temperature_cell_processing(self):
        return {
            'name': _('High Temperature'),
            'type': 'ir.actions.act_window',
            'view_mode': 'list,form',
            'res_model': 'high.temperature.cell',
            'domain': [('id', '=', self.high_temperature_id.id)]
        }
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = seq.next_by_code('manufacturing.cell.injection') or _('New')
        ...
        for rec in self:
            if rec.injection_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.injection_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.injection_id.name,
                    'product_id': rec.product_id.id,
                    ...
                    'company_id': rec.env.company.id,
                    'injection_id': rec.injection_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.injection_id:
                if rec.type == 'HOLD':
                    rec.injection_id.state = 'hold'
                else:
                    rec.injection_id.state = 'progress'
