...
class MachineData(models.Model):
    ...
    voltage_test_value_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')])
...
class Utilities(models.Model):
    ...
    voltage_test_id = fields.Many2one('voltage.test')
    voltage_tester_utility_condition = fields.Selection([
        ('good', 'Good'),
        ('fail', 'Fail')
    ], compute='_compute_voltage_test_cell_utility')

    def _compute_voltage_test_cell_utility(self):
        for rec in self:
            rec.voltage_tester_utility_condition = 'fail'
            if rec.voltage_test_id:
                if rec.voltage_test_id.operation_id.min_humidity <= rec.humidity <= rec.voltage_test_id.operation_id.max_humidity and rec.voltage_test_id.operation_id.min_temperature <= rec.temperature <= rec.voltage_test_id.operation_id.max_temperature:
                    rec.voltage_tester_utility_condition = 'good'
                else:
                    rec.voltage_tester_utility_condition = 'fail'
...
class StockMoveLIne(models.Model):
    ...
    voltage_test_id = fields.Many2one('voltage.test')
    out_voltage_test_id = fields.Many2one('voltage.test')
...
class ManufacturingComponents(models.Model):
    ...
    def _onchange_voltage_test_product_id(self):
        if self.voltage_test_id:
            self.location_src_id = self.voltage_test_id.location_src_id.id
            self.location_dest_id = self.voltage_test_id.location_dest_id.id
...
class VoltageTest(models.Model):
    ...
    def default_get(self, fields):
        defaults = super(VoltageTest, self).default_get(fields)
    ...
    lot_ids = fields.One2many('product.serial.number', 'voltage_test_id', string='Serial Number')
    ...
    component_ids = fields.One2many(
        'manufacturing.component', 'voltage_test_id', 'Components',
        copy=False)
    ...
    machine_data_ids = fields.One2many(
        'machine.parameter', 'voltage_test_id', 'Machine Data',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    utility_ids = fields.One2many(
        'utility.parameter', 'voltage_test_id', 'Utility Parameter',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    quality_ids = fields.One2many(
        'quality.parameter', 'voltage_test_id', 'Quality',
        copy=False, states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    breakdown_ids = fields.One2many('production.breakdown', 'voltage_test_id')
    ...
    aged_formation_count = fields.Integer(compute='_compute_aged_formation_count')
    aged_formation_2_count = fields.Integer(compute='_compute_aged_formation_2_count')
    packing_count = fields.Integer(compute='_compute_packing_count')
    finished_move_ids = fields.One2many('stock.move.line', 'out_voltage_test_id')
    expected_end_time = fields.Datetime("Expected End", compute='compute_end_date')
    out_tray_id = fields.Many2one('product.tray')
    input_material_lines = fields.One2many('material.line', 'voltage_test_id')
    remaining_hours = fields.Float("Remaining Time", compute='_get_remaining_time')
    fai_status_voltage = fields.Boolean(compute='_compute_fai_status_voltage')
    process_status_voltage = fields.Boolean(compute='_compute_process_status_voltage')
    ...
    capacity_test_count_1 = fields.Integer(compute='_compute_capacity_test_count_1')
    ...
    def _compute_fai_status_voltage(self):
        for rec in self:
            fai = self.env['first.article.inspection'].search([('origin', '=', rec.name), ('state', '=', 'done')])
            if fai:
                rec.fai_status_voltage = True
            else:
                rec.fai_status_voltage = False


    @api.depends('pa_count')
    def _compute_process_status_voltage(self):
        for rec in self:
            process = self.env['first.article.inspection'].search(
                [('origin', '=', rec.name), ('state', '=', 'done'), ('inspection_type', '=', 'process')])
            if process:
                rec.process_status_voltage = True
            else:
                rec.process_status_voltage = False
    ...
    def action_view_quality(self):
        return {
            ...
            'domain': [('voltage_test_id', '=', self.id)],
            'view_mode': 'list,form',
        }
    ...
    def _compute_aged_formation_count(self):
        for rec in self:
            rec.aged_formation_count = self.env['aged.formation.cell'].search_count([('voltage_test_id', '=', rec.id)])

    def _compute_capacity_test_count_1(self):
        for rec in self:
            rec.capacity_test_count_1 = self.env['capacity.test'].search_count([('voltage_test_id', '=', rec.id)])

    def _compute_capacity_test_count(self):
        for rec in self:
            rec.capacity_test_count = self.env['capacity.test'].search_count([('id', '=', rec.capacity_id.id)])

    def action_capacity_record(self):
        records = self.env['capacity.test'].search([('id', '=', self.capacity_id.id)])
        if len(records) == 1:
            return {
            ...
            return {
                ...
                'view_mode': 'list,form',
                'res_model': 'capacity.test',
                'domain': [('id', '=', self.capacity_id.id)]
            }

    def show_aged_formation_record(self):
        records = self.env['aged.formation.cell'].search([('voltage_test_id', '=', self.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
                'name': _('Aged Formation'),
                'type': 'ir.actions.act_window',
                'view_mode': 'form',
                'res_model': 'aged.formation.cell',
                'res_id': records.id,  # Assuming records is a single record
            }
        else:
    ...
    def action_create_aged_formation_cell(self):
        aged_formation_cell = self.env['aged.formation.cell'].create({
            'voltage_test_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id,
            'type': 'aged_formation_cell_2',
        })
        aged_formation_cell._onchange_of_product_plan_id()
        return {
            'name': _('Aged Formation'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'aged.formation.cell',
            'res_id': aged_formation_cell.id
        }

    def _compute_aged_formation_2_count(self):
        for rec in self:
            rec.aged_formation_2_count = self.env['aged.formation.cell'].search_count([('id', '=', rec.aged_formation_id.id)])

    def _compute_packing_count(self):
        for rec in self:
            rec.packing_count = self.env['package.move'].search_count([('voltage_test_id', '=', rec.id)])

    def show_aged_formation_2_record(self):
        records = self.env['aged.formation.cell'].search([('id', '=', self.aged_formation_id.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
                'name': _('Aged Formation'),
            ...
            return {
                ...
                'type': 'ir.actions.act_window',
                'view_mode': 'list,form',
                'res_model': 'package.move',
                'domain': [('id', '=', self.aged_formation_id.id)],
            }

    def show_package_record(self):
        records = self.env['package.move'].search([('voltage_test_id', '=', self.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
                'name': _('Package'),
    ...
    def action_create_package_move_cell(self):
        package_move = self.env['package.move'].create({
            'voltage_test_id': self.id,
            'production_plan_id': self.production_plan_id.id,
            'product_model_id': self.product_model_id.id
        })
        package_move._onchange_of_product_plan_id()
        return {
            'name': _('Package'),
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'res_model': 'package.move',
            'res_id': package_move.id
        }
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('name') or vals.get('name') == _('New'):
                vals['name'] = seq.next_by_code('voltage.test') or _('New')
        ...
        for rec in self:
            ...
            return {
                ...
                'context': {
                    ...
                    'default_voltage_test_id': rec.id,
                },
            }
    ...
    def action_create_capacity_test_cell(self):
        capacity_test = self.env['capacity.test'].create({
            ...
            'fai_status_capacity': False,
            'process_status_capacity': False
        })
    ...
    def show_capacity_test_record(self):
        records = self.env['capacity.test'].search([('voltage_test_id', '=', self.id)])
        if len(records) == 1:
            # If there is only one record, return the form view directly
            return {
                'name': _('Capacity test'),
        ...
        for rec in self:
            if rec.voltage_test_id:
                ...
                picking = self.env['stock.picking'].create({
                    ...
                    'move_type': 'direct',
                    'origin': rec.voltage_test_id.name,
                })
                stock_move = self.env['stock.move'].create({
                    'inventory_name': rec.voltage_test_id.name,
                    'product_id': rec.product_id.id,
                    ...
                    'company_id': rec.env.company.id,
                    'voltage_test_id': rec.voltage_test_id.id,
...
class ProductionBreakdown(models.Model):
    ...
    def action_done_breakdown(self):
        for rec in self:
            if rec.voltage_test_id:
                if rec.type == 'HOLD':
                    rec.voltage_test_id.state = 'hold'
                else:
                    rec.voltage_test_id.state = 'progress'
