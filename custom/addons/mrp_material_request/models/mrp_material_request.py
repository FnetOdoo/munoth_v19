...
class MaterialRequestConfiguration(models.Model):
    ...
    name = fields.Char( string='Sub Department Name')
...
class MrpMaterialRequest(models.Model):
    ...
    approval_line_ids = fields.One2many('material.request.configuration', 'material_request_id', string="")
    ...
    is_return_created  = fields.Boolean(copy=False)
    is_transfer_complete = fields.Boolean(compute='_compute_is_transfer_complete')
    production_process_id = fields.Many2one('manufacturing.process')

    is_maintenance_material_request = fields.Boolean()
    ...
    def onchange_equipment(self):
        if self.equipment_id and self.is_maintenance_material_request:
            self.location_dest_id = self.equipment_id.location_id.id or self.picking_type_id.default_location_dest_id.id
    #
    ...
    def action_view_mrp(self):
        self.ensure_one()
        return {
            'name': _('Manufacturing Process'),
            'type': 'ir.actions.act_window',
            'res_model': 'stock.picking',
            'view_mode': 'form',
            'res_id': self.mrp_material_id.id,
            'views': [(False, 'form')],
            'target': 'current',
        }
    ...
    @api.onchange('sub_department_id', 'is_maintenance_material_request', 'category_id')
    def _onchange_populate_approval_lines(self):
        self.approval_line_ids = [(5, 0, 0)]

        if self.sub_department_id and not self.is_maintenance_material_request:
            config = self.sub_department_id.approval_line_id
            approval_line_vals = []
        ...
        elif self.is_maintenance_material_request:
            config = self.category_id.approval_line_id
            approval_line_vals = []
            for user in config.user_ids:
                approval_line_vals.append((0, 0, {
                    'name': user.name,
    ...
    def _compute_location_dest_id(self):
        for rec in self:
            if not rec.production_process_id and not self.is_maintenance_material_request:
                if rec.type == 'in_ward':
                    rec.location_id = rec.sub_department_id.location_id
                    rec.location_dest_id = rec.sub_department_id.location_dest_id
                else:
                    rec.location_id = rec.sub_department_id.location_dest_id
                    rec.location_dest_id = rec.sub_department_id.location_id
            else:
                rec.location_id = rec.sub_department_id.location_id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                if vals.get('is_maintenance_material_request'):
                    vals['name'] = self.env['ir.sequence'].sudo().next_by_code(
                        'maintenance.material.request'
                    ) or _('New')
                elif vals.get('type') == 'out_ward':
                    vals['name'] = self.env['ir.sequence'].sudo().next_by_code(
                        'mrp.material.return'
                    ) or _('New')
                else:
                    vals['name'] = self.env['ir.sequence'].sudo().next_by_code(
                        'mrp.material.request'
                    ) or _('New')
    ...
    def action_check_and_transfer(self):
        self.is_delivery_created = True
        for rec in self:
            picking_id = self.env['stock.picking'].create({
                'picking_type_id': rec.picking_type_id.id,
                ...
                'company_id': rec.env.company.id,
                'origin': rec.name,
                'material_return_id': rec.id,
                'state': 'draft',
            })
            product_lines = self.request_line_ids
            ...
            rec.state = 'material_return_progress'
            rec.is_return_created = True
    ...
    def action_request(self):
        ...
        if self.is_maintenance_material_request :
            if not self.category_id:
                raise UserError(_("Please Select the Category."))
            if not self.order_id:
                raise UserError(_("Please Select the Work Order."))
            if not self.equipment_id:
                raise UserError(_("Please Select the Equipment."))
        ...
        for rec in self:
            ...
            if manager.partner_id.email:
                ...
                if rec.is_maintenance_material_request:
                    # ---------- Maintenance Material Request Email ----------
                    subject = 'Approval Required: Maintenance Material Request %s' % rec.name
                    header_title = 'Maintenance Material Request Approval'
                    ...
                    <div style="font-family: Arial, sans-serif; margin: 0 auto; 
                        ...
                        <!-- Header -->
                        <div style="background-color: #8B2500; padding: 24px 32px;">
                        ...
                        <div style="padding: 32px; background-color: #ffffff;">
                            ...
                            <p style="color: #444444; font-size: 14px; line-height: 1.6;">
                                A new <strong>maintenance material request</strong> has been submitted and is pending your approval.
                                Please review the details below:
                            </p>

                            <!-- Info Card -->
                            <div style="background-color: #fbeee6; border-left: 4px solid #8B2500; 
                            ...
                            <p style="color: #8B2500; font-size: 15px; font-weight: bold; margin: 0;">
                                %s
                            </p>
                        </div>

                        <!-- Footer -->
                        <div style="background-color: #8B2500; padding: 16px 32px; 
                                    border-top: 1px solid #5e1800; text-align: center;">
                            <p style="color: #f0e0d8; font-size: 12px; margin: 0;">
                    ...
                    """ % (
                        ...
                        manager.name,  # Dear
                        rec.name,  # Reference
                        rec.category_id.name,  # Maintenance Category
                    ...
                    # ---------- Regular Material Request Email (unchanged) ----------
    ...
    def action_return_request(self):
        ...
        for line in self.request_line_ids.filtered(lambda l: not l.display_type):
            ...
            if qty > available_qty:
                ...
                ) % (line.product_id.display_name, qty, available_qty))
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
            ...
            if manager.partner_id.email:
                ...
                <div style="font-family: Arial, sans-serif; margin: 0 auto; 
                    ...
                    <div style="padding: 32px; background-color: #ffffff;">
                        ...
                        <p style="color: #444444; font-size: 14px; line-height: 1.6;">
                            A new material return has been submitted and is pending your approval.
                            Please review the details below:
                        </p>
                ...
                mail_values = {
                    ...
                    'email_to': manager.partner_id.email_formatted,
                    'body_html': body_html,
                    'subject': 'Approval Required: Material Return %s' % rec.name,
                }
                self.env['mail.mail'].sudo().create(mail_values).send()
                first_approval.write({
                    'is_email_sent': True,
        ...
        for rec in self:
            ...
            if next_line:
                ...
                if manager.partner_id.email:
                    ...
                    if rec.is_maintenance_material_request:
                        # ---------- Maintenance: Next Approver Mail ----------
                        subject = 'Approval Required: Maintenance Material Request %s' % rec.name
                        body_html = """
                                      <div style="font-family: Arial, sans-serif; margin: 0 auto; 
                                                  border: 1px solid #c0c0c0; border-radius: 8px; overflow: hidden;">
                                          ...
                                          <div style="background-color: #8B2500; padding: 24px 32px;">
                                              <h2 style="color: #ffffff; margin: 0; font-size: 20px; letter-spacing: 0.5px;">
                                                  Maintenance Material Request Approval
                                              </h2>
                                          </div>
                            ...
                            manager.name,  # Dear
                            rec.name,  # Reference
                            rec.category_id.name or '',  # Maintenance Category
                            rec.env.user.name,  # Requested By
                            rec.responsible_user_id.name or '',  # Responsible User
                            rec.create_date.strftime('%d-%m-%Y') if rec.create_date else '',  # Request Date
                            base_url,
                        ...
                        # ---------- Regular: Next Approver Mail (unchanged) ----------
                        subject = 'Approval Required: Material Request %s' % rec.name
                        body_html = """
                                      <div style="font-family: Arial, sans-serif; margin: 0 auto; 
                                                  border: 1px solid #c0c0c0; border-radius: 8px; overflow: hidden;">
            ...
            if rec.is_maintenance_material_request:
                # ---------- Maintenance: Final Approval Mail to Requester ----------
                final_subject = 'Approval Confirmation: Maintenance Material Request %s' % rec.name
                final_header = 'Maintenance Material Request Approved'
            else:
                # ---------- Regular: Final Approval Mail to Requester (unchanged) ----------
                final_subject = 'Approval Confirmation: Material Request %s' % rec.name
                final_header = 'Material Request Approved'
            ...
            if rec.is_maintenance_material_request:
                store_subject_prefix = 'Maintenance Material Request Ready For Store Process'
                store_header = 'Maintenance Material Request Ready For Store Process'
                store_intro = 'Maintenance Material Request'
            else:
                store_subject_prefix = 'Material Request Ready For Store Process'
                store_header = 'Material Request Ready For Store Process'
                store_intro = 'Material Request'
            ...
            for user in store_users:
                ...
                <div style="padding: 32px; background-color: #ffffff;">
                    ...
                    <p style="color: #444444; font-size: 14px; line-height: 1.6;">
                        %s <strong>%s</strong> has completed all approval levels.
                ...
                self.env['mail.mail'].sudo().create({
                    ...
                    'subject': '%s - %s' % (store_subject_prefix, rec.name),
                    'body_html': body_html,
                }).send()
    ...
    def action_return_approve(self):
        for rec in self:
            ...
            if next_line:
                ...
                # Wait for next approver, move on to next record (if any)
                continue
            ...
            body_html = """
               <div style="font-family: Arial, sans-serif; margin: 0 auto; 
                   ...
                   <div style="padding: 32px; background-color: #ffffff;">
                       ...
                       <p style="color: #444444; font-size: 14px; line-height: 1.6;">
                           We are pleased to inform you that your material return has been 
                           <strong style="color: #1a6b3c;">approved</strong> by the manager.
                           Please find the details below:
                       </p>
            ...
            mail_values = {
                ...
                'email_to': rec.user_id.partner_id.email_formatted,
                'body_html': body_html,
                'subject': 'Approval Confirmation: Material Return %s' % rec.name,
            }
            ...
            for user in store_users:
                ...
                <div style="background-color: #1a6b3c; padding: 24px 32px;">
                    <h2 style="color: #ffffff; margin: 0; font-size: 20px; letter-spacing: 0.5px;">
                        Material Return Ready For Store Process
                    </h2>
                </div>
                ...
                <div style="padding: 32px; background-color: #ffffff;">
                    ...
                    <p style="color: #444444; font-size: 14px; line-height: 1.6;">
                        Material Return <strong>%s</strong> has completed all approval levels.
                        Kindly proceed with the store processing.
                    </p>
                ...
                self.env['mail.mail'].sudo().create({
                    ...
                    'email_to': user.partner_id.email_formatted,
                    'subject': 'Material Return Ready For Store Process - %s' % rec.name,
                    'body_html': body_html,
                }).send()
    ...
    def action_purchase_reject(self):
        for rec in self:
            ...
            store_users = self.env['res.users'].sudo().search(
                [('group_ids', 'in', [store_group.id]), ('active', '=', True), ])
            base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
            base_url += '/web#id=%d&view_type=form&model=%s' % (self.id, self._name)
            for user in store_users:
                if not user.partner_id.email:
...
class MrpMaterialRequestLine(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.request_id.production_process_id:
                raise UserError(_("You cannot remove lines when the Material Request is from Manufacturing Process!"))
            if rec.request_id and rec.request_id.state not in ['draft', 'cancel']:
                raise UserError(_("Invalid Action!"))
        return super(MrpMaterialRequestLine, self).unlink()
