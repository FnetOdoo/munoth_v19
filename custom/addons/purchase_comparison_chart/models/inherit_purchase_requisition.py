            ...
            if vals.get('requisition_id'):
                ...
                for po_id in purchase_ids:
                    if vals.get('partner_id') == po_id.partner_id.id:
                        raise UserError(_(
                            'RFQ is available for this purchase agreement for the same vendor'
                        ))
...
# Safe slugify that works in any Odoo version
def slugify(text):
...
class PurchaseRequisition(models.Model):
    ...
    def _compute_url(self):
        ...
        for record in self:
            slug_str = slugify(record.display_name or str(record.id))
            record.print_url = urljoin(
                base_url,
                "purchase_comparison_chart/purchase_comparison/%s-%s" % (
                    slug_str,
                    record.id,
                )
            )
...
class PaymentWizard(models.TransientModel):
    ...
    attachment = fields.Binary('Excel Report File',readonly=True)
