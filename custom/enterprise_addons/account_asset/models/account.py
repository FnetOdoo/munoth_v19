...
class AccountAccount(models.Model):
    ...
    # Migration v15→v19:
    # user_type_id (Many2one to account.account.type) was removed in v16+.
    # Account classification is now done via account_type (Selection) directly on account.account.
    # asset_type values map to account_type as follows:
    #   'purchase' ← account_type in ('asset_fixed', 'asset_non_current')
    #   'sale'     ← account_type in ('liability_current', 'liability_non_current')
    #   'expense'  ← account_type in ('asset_current', 'asset_prepayments')
    ...
    def auto_generate_asset(self):
        # Migration v15→v19: user_type_id removed; use account_type Selection directly
        return self.account_type in ('asset_fixed', 'asset_non_current')

    def auto_generate_deferred_revenue(self):
        # Migration v15→v19: user_type_id removed; use account_type Selection directly
        return self.account_type in ('liability_current', 'liability_non_current')

    def auto_generate_deferred_expense(self):
        # Migration v15→v19: user_type_id removed; use account_type Selection directly
        return self.account_type in ('asset_current', 'asset_prepayments')

    def get_asset_accounts_type(self):
        # Migration v15→v19: formerly returned account.account.type records (env.ref)
        # Now returns the account_type string values directly
        return ('asset_fixed', 'asset_non_current')

    def get_deferred_revenue_accounts_type(self):
        # Migration v15→v19: formerly returned account.account.type records (env.ref)
        return ('liability_current', 'liability_non_current')

    def get_deferred_expense_accounts_type(self):
        # Migration v15→v19: formerly returned account.account.type records (env.ref)
        return ('asset_current', 'asset_prepayments')
