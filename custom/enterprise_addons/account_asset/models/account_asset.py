...
import calendar
# Migration v15→v19: removed erroneous 'from encodings.punycode import digits'
...
class AccountAsset(models.Model):
    ...
    # Depreciation params
    method = fields.Selection([('linear', 'Straight Line'), ('degressive', 'Declining'), ('degressive_then_linear', 'Declining then Straight Line')], string='Method', readonly=True, default='linear',
    ...
    method_number = fields.Integer(string='Number of Depreciations', readonly=True, default=5, help="The number of depreciations needed to depreciate your asset")
    method_period = fields.Selection([('1', 'Months'), ('12', 'Years')], string='Number of Months in a Period', readonly=True, default='12',
        help="The amount of time between two depreciations")
    method_progress_factor = fields.Float(string='Declining Factor', readonly=True, default=0.3, digits=(6,4))
    ...
    prorata_date = fields.Date(
        string='Prorata Date',
        readonly=True)
    account_asset_id = fields.Many2one('account.account', string='Fixed Asset Account', compute='_compute_account_asset_id', help="Account used to record the purchase of the asset at its original price.", store=True, domain="[('company_ids', 'in', company_id), ('is_off_balance', '=', False)]")
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Account', readonly=True, domain="[('deprecated', '=', False), ('company_ids', 'in', company_id), ('is_off_balance', '=', False)]", help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Expense Account', readonly=True, domain="[('deprecated', '=', False), ('company_ids', 'in', company_id), ('is_off_balance', '=', False)]", help="Account used in the periodical entries, to record a part of the asset as expense.")

    journal_id = fields.Many2one('account.journal', string='Journal', readonly=True, domain="[('type', '=', 'general'), ('company_id', '=', company_id)]")

    # Values
    original_value = fields.Monetary(string="Original Value", compute='_compute_value', store=True, readonly=True)
    ...
    original_move_line_ids = fields.Many2many('account.move.line', 'asset_move_line_rel', 'asset_id', 'line_id', string='Journal Items', readonly=True, copy=False)
    ...
    # analytic_tag_ids removed: account.analytic.tag model was dropped in Odoo 17+
    ...
    disposal_date = fields.Date(readonly=True, compute="_compute_disposal_date", store=True)

    # model-related fields
    model_id = fields.Many2one('account.asset', string='Model', change_default=True, readonly=True, domain="[('company_id', '=', company_id)]")
    # Migration v15→v19: user_type_id (account.account.type) removed in v16+
    # account_type is a Selection field on account.account in Odoo 17+
    account_type = fields.Selection(related='account_asset_id.account_type', string='Type of the account')
    ...
    # Migration v15→v19: removed user_type_id from depends (field removed)
    @api.depends('asset_type', 'account_type', 'state')
    def _compute_display_model_choice(self):
        for record in self:
            domain = [('state', '=', 'model')]
            # Migration v15→v19: user_type_id removed; filter by account_type
    ...
    def _onchange_account_asset_id(self):
        # Migration v15→v19: user_type_id removed
    ...
    def _recompute_board(self, depreciation_number, starting_sequence, amount_to_depreciate, depreciation_date, already_depreciated_amount, amount_change_ids):
        ...
        if amount_to_depreciate != 0.0:
            for asset_sequence in range(starting_sequence + 1, depreciation_number):
