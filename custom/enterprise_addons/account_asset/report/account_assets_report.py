...
# Migration v15→v19:
# account.report changed from AbstractModel to concrete Model in v17+.
# An AbstractModel cannot inherit a concrete Model — caused:
#   TypeError: abstract model 'account.assets.report' cannot inherit
#   from non-abstract model 'account.report'
# Fix: use models.Model (not AbstractModel); both _name and _inherit kept.
class assets_report(models.Model):
    _inherit = 'account.report'
    _name = 'account.assets.report'
    _description = 'Account Assets Report'

    # filter_date, filter_all_entries, filter_hierarchy removed:
    # these were AbstractModel-era class attributes not valid as ORM fields in Odoo 17+.

    # Override ALL inherited Many2many fields to use unique relation table names.
    # Required because account.assets.report is a new concrete model (_name != _inherit),
    # so Odoo creates a separate table but would otherwise clash with account.report's tables.
    section_report_ids = fields.Many2many(
        comodel_name='account.report',
        relation='account_assets_report_section_rel',
        column1='report_id',
        column2='section_id',
        string='Sections',
    )
    section_main_report_ids = fields.Many2many(
        comodel_name='account.report',
        relation='account_assets_report_section_main_rel',
        column1='section_id',
        column2='report_id',
        string='Main Reports',
    )
    line_ids = fields.Many2many(
        ...
        relation='account_assets_report_line_rel',
        column1='report_id',
        column2='line_id',
        string='Lines',
    )
    column_ids = fields.Many2many(
        ...
        relation='account_assets_report_column_rel',
        column1='report_id',
        column2='column_id',
        string='Columns',
    )
    ...
    def _get_account_group_with_company(self, account_code, company_id, parent_group=None, group_dict=None):
        ...
        if not account_code:
            ...
            account_code = '##'
        # Migration v15→v19: _get_account_groups_for_asset_report may need update per your v19 account_reports version
    ...
    def _get_assets_lines(self, options):
        ...
        # Migration v15→v19: self.flush() renamed to self.env.flush_all() in v17+
        self.env.flush_all()
