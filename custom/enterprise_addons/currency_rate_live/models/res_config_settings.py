...
#
# Migrated from Odoo 15 → Odoo 19
# Changes:
#   - Removed import of xml2json_from_elementtree from odoo.addons.web.controllers.main
#     (that helper was removed in Odoo v18/v19). Inlined an equivalent implementation.
#   - Replaced deprecated `attrs` attribute in XML views with `invisible` (handled in view).
#   - Updated version in __manifest__.py to 19.0.x.x.x.
#   - ResCompany.create() → uses super() without explicit class name (Python 3 style kept,
#     but no functional change needed as Odoo v19 still accepts both).
...
# ---------------------------------------------------------------------------
# Inline replacement for xml2json_from_elementtree
# Previously imported from odoo.addons.web.controllers.main, which was removed
# in Odoo v18/v19. This is a faithful reimplementation of the original helper.
# ---------------------------------------------------------------------------
def xml2json_from_elementtree(el, preserve_whitespaces=False):
    """Recursively convert an lxml/ElementTree element into a dict structure
    compatible with the format the original Odoo helper produced:

        {
            'tag': str,
            'namespace': str (optional),
            'attrs': dict,
            'children': list,   # strings or nested dicts
        }
    """
    res = {}
    # Handle namespaced tags like '{http://...}tagname'
    if el.tag and el.tag[0] == '{':
    ...
    res['attrs'] = dict(el.attrib)
    res['children'] = []

    def _add_text(text):
        if preserve_whitespaces:
            if text:
                res['children'].append(text)
        else:
            if text and text.strip():
                res['children'].append(text.strip())

    _add_text(el.text)

    for child in el:
        res['children'].append(xml2json_from_elementtree(child, preserve_whitespaces))
        # tail text belongs to the parent's children list
        if preserve_whitespaces:
            if child.tail:
                res['children'].append(child.tail)
        else:
            if child.tail and child.tail.strip():
                res['children'].append(child.tail.strip())
...
class ResCompany(models.Model):
    ...
    def create(self, vals_list):
        ...
        Note: v19 prefers @api.model_create_multi receiving a list of dicts.
        """
        code_providers = {
            'CH': 'fta', 'MX': 'banxico', 'CA': 'boc',
            'RO': 'bnr', 'CL': 'mindicador', 'PE': 'bcrp', 'AE': 'cbuae',
        }
    ...
    def update_currency_rates(self):
        """Update all currencies given by the provider.

        Calls _parse_<provider>_data() for the selected provider, then stores
        the resulting rates via _generate_currency_rates().

        :return: True if all records were updated successfully, False otherwise.
        """
        rslt = True
        ...
        for currency_provider, companies in self._group_by_provider().items():
            ...
            if parse_results is False:
                _logger.warning(
                    'Unable to connect to the online exchange rate platform %s. '
    ...
    def _group_by_provider(self):
        """Return a dict grouping companies by currency rate provider.
        Companies with no provider defined are ignored.
        """
        rslt = {}
        for company in self:
            if not company.currency_provider:
                continue
            rslt.setdefault(company.currency_provider, self.env['res.company'])
            rslt[company.currency_provider] += company
        return rslt

    def _generate_currency_rates(self, parsed_data):
        """Generate currency rate entries for each company based on parsed_data.

        Rates are normalised so the company's main currency always has rate=1.
        """
        ...
        for company in self:
            rate_info = parsed_data.get(company.currency_id.name)
            if not rate_info:
                raise UserError(_(
    ...
    def _parse_fta_data(self, available_currencies):
        """Parse XML data from the Swiss Federal Tax Administration."""
        request_url = 'https://www.backend-rates.ezv.admin.ch/api/xmldaily?d=today&locale=en'
        try:
            parse_url = requests.request('GET', request_url, timeout=30)
        except Exception:
            return False
            ...
            if child_node['tag'] == 'devise':
                ...
                if currency_code in available_currency_names:
                    ...
                    rates_dict[currency_code] = (
                        float(re.search(r'\d+', currency_xml).group()) / float(rate_xml),
                        fields.Date.today(),
                    )
    ...
    def _parse_ecb_data(self, available_currencies):
        """Parse XML data from the European Central Bank. Rates are against EUR."""
        ...
        available_currency_names = available_currencies.mapped('name')
        rslt = {
            x['attrs']['currency']: (float(x['attrs']['rate']), fields.Date.today())
    ...
    def _parse_cbuae_data(self, available_currencies):
        """Parse HTML data from the UAE Central Bank.
        Rates are expressed as 1 foreign currency unit → AED.
        """
        try:
    ...
    def _parse_boc_data(self, available_currencies):
        """Parse JSON data from the Bank of Canada.
        Rates are expressed as 1 foreign currency unit → CAD.
        Keys format: 'FX{CODE}CAD' e.g. 'FXEURCAD'.
        """
        available_currency_names = available_currencies.mapped('name')
        request_url = "http://www.bankofcanada.ca/valet/observations/group/FX_RATES_DAILY/json"
        try:
            response = requests.request('GET', request_url, timeout=30)
        except Exception:
            return False
        ...
        last_obs = next(obs for obs in data['observations'] if obs['d'] == last_observation_date)
        ...
        for currency_name in available_currency_names:
            currency_obs = last_obs.get('FX{}CAD'.format(currency_name))
    ...
    def _parse_banxico_data(self, available_currencies):
        """Parse JSON data from Banco de México (Banxico).

        The base currency is MXN. Supported series:
            SF46410 EUR, SF60632 CAD, SF46406 JPY, SF46407 GBP, SF60653 USD
        """
        ...
        try:
            ...
            series = {
                serie['idSerie']: {dato['fecha']: dato['dato'] for dato in serie['datos']}
                for serie in series if 'datos' in serie
            }
        except Exception:
            return False
        ...
        today_fmt = date_mx.strftime(BANXICO_DATE_FORMAT)
        yesterday_fmt = (date_mx - datetime.timedelta(days=1)).strftime(BANXICO_DATE_FORMAT)

        for index, currency in foreigns.items():
            if not series.get(index):
                continue
    ...
    def _parse_xe_com_data(self, available_currencies):
        """Parse HTML data from xe.com. Rates are relative to USD."""
        url_format = 'http://www.xe.com/currencytables/?from=%(currency_code)s'
        today = fields.Date.today()
        try:
            fetched_data = requests.request(
                'GET', url_format % {'currency_code': 'USD'}, timeout=30
            )
        except Exception:
            return False
    ...
    def _parse_bnr_data(self, available_currencies):
        """Parse XML data from the National Bank of Romania. Rates are against RON."""
        ...
        if rates_node:
            rate_date = (
                datetime.datetime.strptime(rates_node['attrs']['date'], DEFAULT_SERVER_DATE_FORMAT)
    ...
    def _parse_bcrp_data(self, available_currencies):
        """Parse JSON data from the Bank of Peru (BCRP).

        API: https://estadisticas.bcrp.gob.pe/estadisticas/series/api
        Supported series:
            PD04640PD  USD → PEN (Venta)
            PD04648PD  EUR → PEN (Venta)
        """
        ...
        url_format = (
            ...
            "/%(currency_code)s/json/%(date_start)s/%(date_end)s/ing"
        )
        foreigns = {
            'USD': 'PD04640PD',
            'EUR': 'PD04648PD',
        ...
        date_pe = (
            self.mapped('currency_next_execution_date')[0]
            or datetime.datetime.now(timezone('America/Lima'))
        )
        ...
        data = {'date_start': first_pe_str, 'date_end': second_pe_str}
        ...
        for currency_odoo_code, currency_pe_code in foreigns.items():
            ...
            if not rate:
                continue
            # Service returns 'Set' instead of the standard 'Sep'
            normalized_date = date_rate_str.replace('Set', 'Sep')
            date_rate = datetime.datetime.strptime(
                normalized_date, bcrp_date_format_res
            ).strftime(DEFAULT_SERVER_DATE_FORMAT)
    ...
    def _parse_mindicador_data(self, available_currencies):
        """Parse JSON data from mindicador.cl (Chile).

        Base currency: CLP. Supported rates: EUR, USD, UF, UTM.
        """
        ...
        for index, currency in foreigns.items():
            if index not in available_currency_names:
                logger.debug('Index %s not in available currency names', index)
                continue
    ...
    def run_update_currency(self):
        """Called from a scheduled action (cron) to update currency rates."""
...
class ResConfigSettings(models.TransientModel):
    ...
    def onchange_currency_interval_unit(self):
        # Avoid overwriting next execution date if it has already been set
    ...
    def update_currency_rates_manually(self):
        ...
        if not self.company_id.update_currency_rates():
            raise UserError(_(
                ...
                'The web service may be temporarily down. Please try again in a moment.'
            ))
