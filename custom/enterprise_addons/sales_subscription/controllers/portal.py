...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'subscription_count' in counters:
            ...
            values['subscription_count'] = (
                request.env['sales.subscription'].search_count(self._get_subscription_domain(partner))
                if request.env['sales.subscription'].check_access_rights('read', raise_exception=False)
                else 0
            )
        return values
    ...
    def my_subscription(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, **kw):
        ...
        partner = request.env.user.partner_id
        SaleSubscription = request.env['sales.subscription']
        ...
        return request.render("sales_subscription.portal_my_subscriptions", values)
...
class SaleSubscription(http.Controller):
    ...
    def subscription(self, subscription_id, access_token='', message='', message_class='', **kw):
        ...
        Subscription = request.env['sales.subscription']
        ...
        action = request.env.ref('sales_subscription.sales_subscription_action')
        values = {
            'account': subscription,
            'template': subscription.template_id.sudo(),
            'display_close': display_close,
            'is_follower': is_follower,
            'close_reasons': request.env['sales.subscription.close.reason'].search([]),
        ...
        return request.render("sales_subscription.subscription", values)
    ...
    def close_account(self, account_id, uuid=None, **kw):
        account_res = request.env['sales.subscription']
        ...
        if account.sudo().template_id.user_closable:
            close_reason = request.env['sales.subscription.close.reason'].browse(int(kw.get('close_reason_id')))
...
class PaymentPortal(payment_portal.PaymentPortal):
    ...
    def subscription_assign_token(self, subscription_id, token_id):
        ...
        :return: None
        """
        subscription = request.env['sales.subscription'].browse(subscription_id)
