...
class AccountAnalyticAccount(models.Model):
    ...
    subscription_ids = fields.One2many('sales.subscription', 'analytic_account_id', string='Subscriptions')
    subscription_count = fields.Integer(compute='_compute_subscription_count', string='Subscription Count')

    def _compute_subscription_count(self):
        subscription_data = self.env['sales.subscription'].read_group(domain=[('analytic_account_id', 'in', self.ids)],
