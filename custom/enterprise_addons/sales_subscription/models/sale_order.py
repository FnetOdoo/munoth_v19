...
class SaleOrder(models.Model):
    ...
    subscription_product = fields.Boolean(string="Subscription", compute='_compute_subscription_product', store=True)

    @api.depends('order_line.product_id.recurring_invoice')
    def _compute_subscription_product(self):
        for order in self:
            order.subscription_product = any(line.product_id.recurring_invoice for line in order.order_line)
    ...
    def action_open_subscriptions(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("sales_subscription.sale_subscription_action")
        if len(subscriptions) > 1:
            action['domain'] = [('id', 'in', subscriptions.ids)]
        elif len(subscriptions) == 1:
            form_view = [(self.env.ref('sales_subscription.sale_subscription_view_form').id, 'form')]
            if 'views' in action:
    ...
    def _prepare_subscription_data(self, template):
        ...
        recurring_next_date = self.env['sales.subscription']._get_recurring_next_date(
        ...
        values = {
            ...
            'pricelist_id': self.pricelist_id.id or self.partner_id.with_company(self.company_id).property_product_pricelist.id or self.env['sales.subscription']._get_default_pricelist(),
        ...
        default_stage = self.env['sales.subscription.stage'].search([('category', '=', 'progress')], limit=1)
        if default_stage:
    ...
    def create_subscriptions(self):
        ...
        for order in self:
            ...
            for template in to_create:
                ...
                subscription = self.env['sales.subscription'].sudo().create(values)
                subscription.onchange_date_start()
                res.append(subscription.id)
                to_create[template].write({'subscription_id': subscription.id})
                subscription.message_post(
                    body=_(
                        "This subscription was created from <a href='#' data-oe-model='sale.order' data-oe-id='%d'>%s</a>")
                         % (order.id, order.name),
...
class SaleOrderLine(models.Model):
    ...
    subscription_id = fields.Many2one('sales.subscription', 'Subscription', copy=False, check_company=True)
    subscription_product = fields.Boolean(string="Subscription", related='product_id.recurring_invoice')
        ...
        if self.subscription_id:
            ...
            if self.order_id.subscription_management != 'upsell':  # renewal or creation: one entire period
                ...
                if not date_start or not date_start_display or not date_end:
                    ...
                    # (based on `partial_recurring_invoice_ratio` from the sales.subscription model)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('order_id'):
                ...
                if order.origin and order.subscription_management in ('upsell', 'renew') and Product.browse(
                        vals['product_id']).recurring_invoice:
                    subscription = self.env['sales.subscription'].search([
                        ('code', '=', order.origin),
                        ('partner_id', '=', order.partner_id.id)
                    ], limit=1)
                    vals['subscription_id'] = subscription.id or False
    ...
    def _update_subscription_line_data(self, subscription):
        ...
        for line in self:
            sub_line = subscription.recurring_invoice_line_ids.filtered(
                lambda l: (l.product_id, l.uom_id, l.price_unit) == (line.product_id, line.product_uom_id, line.price_unit)
            )
            if sub_line:
