...
class SaleSubscriptionCloseReason(models.Model):
    _name = "sales.subscription.close.reason"
    _order = "sequence, id"
...
class SaleSubscriptionTemplate(models.Model):
    _name = "sales.subscription.template"
    ...
    #     'account.analytic.tag', 'sales_subscription_template_tag_rel',
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref('sales_subscription.mail_template_subscription_invoice', raise_if_not_found=False))
    ...
    def _compute_subscription_count(self):
        subscription_data = self.env['sales.subscription'].read_group(domain=[('template_id', 'in', self.ids), ('stage_id', '!=', False)],
                                                                      fields=['template_id'],
                                                                      groupby=['template_id'])
...
class SaleSubscription(models.Model):
    ...
    stage_id = fields.Many2one('sales.subscription.stage',string='Stage',index=True,
        default=lambda self: self._get_default_stage_id(), copy=False, group_expand='_read_group_stage_ids', tracking=True)

    def _read_group_stage_ids(self, stages, domain):
        return self.env['sales.subscription.stage'].sudo().search([],)

    def _get_default_stage_id(self):
        return self.env['sales.subscription.stage'].search([], order='sequence', limit=1)
    ...
    tag_ids = fields.Many2many('account.analytic.account', string='Tags',
    ...
    recurring_invoice_line_ids = fields.One2many('sales.subscription.line', 'analytic_account_id', string='Subscription Lines', copy=True)
    ...
    recurring_rule_type = fields.Selection([('daily', 'Days'), ('weekly', 'Weeks'),
                                           ...
                                           help="Invoice automatically repeat at specified interval",
                                           default='monthly', tracking=True)
    recurring_interval = fields.Integer(string='Repeat Every', help="Repeat every (Days/Week/Month/Year)", readonly=1)
    # recurring_interval
    ...
    close_reason_id = fields.Many2one("sales.subscription.close.reason", string="Close Reason", copy=False, tracking=True)
    template_id = fields.Many2one(
        'sales.subscription.template', string='Subscription Template',
        required=True,
        help="The subscription template defines the invoice policy and the payment terms.", tracking=True, check_company=True)
    subscription_log_ids = fields.One2many('sales.subscription.log', 'subscription_id', string='Subscription Logs', readonly=True)
    payment_mode = fields.Selection([
        ('manual', 'Manually'),
        ('draft_invoice', 'Draft'),
        ('validate_send', 'Post'),
    ...
    # sale_type_line_id = fields.Many2one('sale.type.line')
    # type = fields.Many2one('renewal.days')
    rating_last_value = fields.Float("Rating (1-5)", aggregator="avg", readonly=True)
    campaign_id = fields.Many2one('utm.campaign', 'Campaign', ondelete='set null')
    source_id = fields.Many2one('utm.source', 'Source', ondelete='set null')
    medium_id = fields.Many2one('utm.medium', 'Medium', ondelete='set null')
    ...
    rating_access_token = fields.Char(
        compute='_compute_rating_access_token',
        store=False
    )

    def _compute_rating_access_token(self):
        for subscription in self:
            subscription.rating_access_token = subscription.rating_get_access_token()

    @api.depends('rating_ids.rating')
    def _compute_percentage_satisfaction(self):
        for subscription in self:
            ratings = subscription.rating_ids.filtered(lambda r: r.rating >= 0)
            if not ratings:
                subscription.percentage_satisfaction = 0.0
                continue
            great = len(ratings.filtered(lambda r: r.rating >= 3))
            okay = len(ratings.filtered(lambda r: r.rating == 2))
            bad = len(ratings.filtered(lambda r: r.rating <= 1))
            total = great + okay + bad

            if total > 0:
                subscription.percentage_satisfaction = round((great * 100.0) / total, 2)
            else:
                subscription.percentage_satisfaction = 0.0
    ...
    def _inverse_starred(self):
        starred_subscriptions = not_star_subscriptions = self.env['sales.subscription'].sudo()
        for subscription in self:
    ...
    def action_open_sales(self):
        ...
        return {
            ...
            "views": [[self.env.ref('sales_subscription.sale_order_view_tree_subscription').id, "list"],
    ...
    def _track_subtype(self, init_values):
        ...
        if 'stage_id' in init_values:
            return self.env.ref('sales_subscription.subtype_stage_change')
    ...
    @api.depends('recurring_total', 'recurring_interval', 'recurring_rule_type')
    def _compute_recurring_monthly(self):
        # Generally accepted ratios for monthly reporting
        for sub in self:
            if sub.recurring_total * INTERVAL_FACTOR[sub.recurring_rule_type] > 0 and sub.recurring_interval > 0:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['code'] = (
                    ...
                    .next_by_code('sales.subscription')
                    or 'New'
            )
            if vals.get('name', 'New') == 'New':
                vals['name'] = vals['code']

            # Set recurring invoice day
            if not vals.get('recurring_invoice_day'):
                sub_date = (
                        vals.get('recurring_next_date')
        ...
        # Post-create logic (must be AFTER super)
    ...
    def _mail_track(self, tracked_fields, initial):
        ...
        if any(f in updated_fields for f in ['recurring_total', 'template_id']):
            ...
            if initial_rrule_type > 0 and initial_rrule_interval > 0:
            ...
            if INTERVAL_FACTOR[self.recurring_rule_type] > 0 and self.recurring_interval > 0:
            ...
            if not float_is_zero(delta, precision_rounding=cur_round):
                self.env['sales.subscription.log'].sudo().create({
    ...
    def _get_subscription_delta(self, date):
        ...
        subscription_log = self.env['sales.subscription.log'].search([
    ...
    def set_close(self):
        ...
        search = self.env['sales.subscription.stage'].search
        for sub in self:
    ...
    def _prepare_invoice_data(self):
        ...
        partner_id = self.partner_invoice_id or addr['invoice']
        partner_shipping_id = self.partner_shipping_id or addr['delivery']
        fpos = self.env['account.fiscal.position'].with_company(company)._get_fiscal_position(self.partner_id, self.partner_id)
    ...
    def prepare_renewal_order(self, discard_product_ids=False, new_lines_ids=False):
        ...
        order.message_post(body=(_("This renewal order has been created from the subscription ") + " <a href=# data-oe-model=sales.subscription data-oe-id=%d>%s</a>" % (self.id, self.display_name)))
    ...
    def start_subscription(self):
        self.ensure_one()
        next_stage_in_progress = self.env['sales.subscription.stage'].search([('category', '=', 'progress'), ('sequence', '>=', self.stage_id.sequence)], limit=1)
        if not next_stage_in_progress:
            next_stage_in_progress = self.env['sales.subscription.stage'].search([('category', '=', 'progress')], limit=1)
    ...
    def _recurring_create_invoice(self, automatic=False, batch_size=20):
        ...
        invalid_payment_tag = self.env.ref('sales_subscription.subscription_invalid_payment', raise_if_not_found=False)
        if automatic:
            if not invalid_payment_tag:
                invalid_payment_tag = self.env['account.analytic.plan'].create({'name': 'Subscription Failed Payment', 'color': 1})
                self.env['ir.model.data'].create({
                    'name': 'subscription_invalid_payment',
                    'module': 'sales_subscription',
                        ...
                        try:
                            ...
                            if tx is None or not tx.renewal_allowed:
                                ...
                                if close_subscription:
                                    template = self.env.ref('sales_subscription.email_payment_close')
                                    ...
                                    template = self.env.ref('sales_subscription.email_payment_reminder')
        ...
        self._cr.execute('''
            ...
            JOIN sales_subscription subscr ON subscr.id = aml.subscription_id
            JOIN sales_subscription_template subscr_tpl ON subscr_tpl.id = subscr.template_id
        ...
        if need_cron_trigger:
            if config['test_enable'] or config['test_file']:
                ...
                self.env['sales.subscription']._recurring_create_invoice(automatic, batch_size)
            else:
                self.env.ref('sales_subscription.account_analytic_cron_for_invoice')._trigger()
    ...
    def send_success_mail(self, tx, invoice):
        ...
        template = self.env.ref('sales_subscription.email_payment_success')
...
class SaleSubscriptionLine(models.Model):
    ...
    analytic_account_id = fields.Many2one('sales.subscription', string='Subscription', ondelete='cascade')
    ...
    uom_id = fields.Many2one('uom.uom', string='Unit of Measure', required=True, domain="[('relative_uom_id', '=', product_uom_category_id)]")
    ...
    def onchange_product_quantity(self):
        if not self.product_id:
            ...
            if not self.uom_id or self.product_id.uom_id.relative_uom_id.id != self.uom_id.relative_uom_id.id:
    ...
    def create(self, vals_list):
        for values in vals_list:
            if values.get('product_id') and not values.get('name'):
                line = self.new(values)
                line.onchange_product_id()
                values['name'] = line._fields['name'].convert_to_write(
                    line['name'], line
                )
...
class SaleSubscriptionStage(models.Model):
    _name = 'sales.subscription.stage'
    ...
    rating_template_id = fields.Many2one('mail.template', string='Rating Email Template',
                                         ...
                                         domain=[('model', '=', 'sales.subscription')])
...
class SaleSubscriptionLog(models.Model):
    _name = 'sales.subscription.log'
    _description = 'Subscription Log'
    _order = 'event_date desc, id desc'

    subscription_id = fields.Many2one(
        'sales.subscription', string='Subscription',
...
class SaleSubscriptionAlert(models.Model):
    _name = 'sales.subscription.alert'
    ...
    def default_get(self, default_fields):
        ...
        if 'model_id' in default_fields:
            ...
            res['model_id'] = self.env['ir.model']._get_id('sales.subscription')
        return res
    ...
    subscription_template_ids = fields.Many2many(
        'sales.subscription.template', string='Subscription Templates',
        check_company=True)
    ...
    stage_from_id = fields.Many2one('sales.subscription.stage', help="Trigger on changes of stage. Trigger over all stages if not set")
    stage_to_id = fields.Many2one('sales.subscription.stage')

    tag_id = fields.Many2one('account.analytic.account', string='Tag')
    stage_id = fields.Many2one('sales.subscription.stage', string='Stage')
    ...
    template_id = fields.Many2one('sales.subscription.template', 'Subscription Template')
    ...
    def _compute_subscription_count(self):
        for alert in self:
            ...
            alert.subscription_count = self.env['sales.subscription'].search_count(domain)
    ...
    def _configure_alert_from_action(self, vals):
        ...
        for alert in self:
            if alert.action == 'set_tag' and alert.tag_id:
                ...
                super(SaleSubscriptionAlert, alert).write({'action': vals.get('action')})
    ...
    def action_view_subscriptions(self):
        ...
        return {
            ...
            'res_model': 'sales.subscription',
            'view_mode': 'kanban,tree,form,pivot,graph,activity',
            'domain': domain,
            'context': {'create': False},
        }
    ...
    def run_cron_manually(self):
        ...
        subs = self.env['sales.subscription'].search(domain)
        ctx = {
            'active_model': 'sales.subscription',
            'active_ids': subs.ids,
            'domain_post': domain,
        }
    ...
    def _compute_nextcall(self):
        cron = self.env.ref('sales_subscription.ir_cron_sales_subscription_update_kpi', raise_if_not_found=False)
        if not cron:
