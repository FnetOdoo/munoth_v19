...
class sales_subscription_report(models.Model):
    _name = "sales.subscription.report"
    ...
    template_id = fields.Many2one('sales.subscription.template', 'Subscription Template', readonly=True)
    ...
    close_reason_id = fields.Many2one('sales.subscription.close.reason', 'Close Reason', readonly=True)
    ...
    stage_id = fields.Many2one('sales.subscription.stage', string='Stage', readonly=True)
    ...
    def _from(self):
        from_str = """
                sales_subscription_line l
                      join sales_subscription sub on (l.analytic_account_id=sub.id)
                      join sales_subscription_stage stage on sub.stage_id = stage.id
