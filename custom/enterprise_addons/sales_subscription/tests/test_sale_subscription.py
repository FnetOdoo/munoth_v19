...
from odoo.addons.sale_subscription.tests.common_sales_subscription import TestSubscriptionCommon
...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_01_template(self):
        ...
        Subscription = self.env['sales.subscription']
        ...
        self.assertTrue(self.subscription.description, 'sales_subscription: description not copied on existing sales.subscription record')
        ...
        self.assertTrue(temp.description, 'sales_subscription: description not copied on new cached sales.subscription record')

    @mute_logger('odoo.addons.base.models.ir_model', 'odoo.models')
    def test_02_sale_order(self):
        """ Test sales order line copying for recurring products on confirm"""
        self.sale_order.action_confirm()
        self.assertTrue(len(self.subscription.recurring_invoice_line_ids.ids) == 1, 'sales_subscription: recurring_invoice_line_ids not created when confirming sale_order with recurring_product')
        self.assertEqual(self.sale_order.subscription_management, 'upsell', 'sales_subscription: so should be set to "upsell" if not specified otherwise')
    ...
    def test_05_sub_creation(self):
        ...
        self.assertEqual(len(self.sale_order_2.order_line.mapped('subscription_id')), 1, 'sales_subscription: subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_2.subscription_management, 'create', 'sales_subscription: subscription creation should set the SO to "create"')

        # Two product with different subscription template
        self.sale_order_3.action_confirm()
        self.assertEqual(len(self.sale_order_3.order_line.mapped('subscription_id')), 2, 'sales_subscription: Two different subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_3.subscription_management, 'create', 'sales_subscription: subscription creation should set the SO to "create"')

        # Two product with same subscription template
        self.sale_order_4.action_confirm()
        self.assertEqual(len(self.sale_order_4.order_line.mapped('subscription_id')), 1, 'sales_subscription: One subscription should be created on SO confirmation')
        self.assertEqual(self.sale_order_4.subscription_management, 'create', 'sales_subscription: subscription creation should set the SO to "create"')

        today = fields.Date.context_today(self.env.user)
        self.sale_order_5.action_confirm()
        self.assertEqual(self.sale_order_5.order_line.subscription_id.date, today + relativedelta(months=+1), 'sales_subscription: subscription creation should set the end date to : today + 1 month')

    @mute_logger('odoo.models.unlink')
    def test_06_renewal(self):
        """ Test subscription renewal """
        discard_ids = [x['product_id'] for x in not_renewed_products]
        res = self.subscription._prepare_renewal_order(discard_product_ids=discard_ids,
        ...
        self.assertEqual(new_line.subscription_id, self.subscription, 'sales_subscription: SO lines added to renewal orders manually should have the correct subscription set on them')
        self.assertEqual(renewal_so.origin, self.subscription.code, 'sales_subscription: renewal order must have the "source document" set to the subscription code')
        self.assertEqual(renewal_so.subscription_management, 'renew', 'sales_subscription: renewal quotation generation is wrong')
        ...
        self.assertEqual(total_quantity, 7, 'sales_subscription: Renewal quantity is not equal to the SO quantity')
        self.assertEqual(renewal_so.subscription_management, 'renew', 'sales_subscription: so should be set to "renew" in the renewal process')

    def test_07_upsell_via_so(self):
        """Test the upsell flow using an intermediary upsell quote."""
        wiz = self.env['sales.subscription.wizard'].create({
        ...
        self.assertEqual(self.subscription, new_line.subscription_id,
                         '''sales_subscription: upsell line added to quote after '''
    ...
    def test_10_compute_kpi(self):
        ...
        self.env['sales.subscription']._cron_update_kpi()
    ...
    def test_11_onchange_date_start(self):
        recurring_bound_tmpl = self.env['sales.subscription.template'].create({
            'name': 'Recurring Bound Template',
            'recurring_rule_boundary': 'limited',
        })
        sub_form = Form(self.env['sales.subscription'])
    ...
    def test_17_renew_archived_products(self):
        ...
        self.assertEqual(archived_product_ids, [self.product2.id], 'sales_subscription: Product 2 should be archived and recognized as such')
        self.assertTrue(archived_product_count == 1, 'sales_subscription: One product should be marked as archived')
        wiz = self.env['sales.subscription.renew.wizard'].create({
        ...
        self.assertEqual(renewal_so.subscription_management, 'renew',
                         'sales_subscription: so should be set to "renew" in the renewal process')
        self.assertEqual(lines_renew[0][1], self.product.id, 'sales_subscription: First product should be TestProduct')
        self.assertEqual(lines_renew[0][2], 7,
                         'sales_subscription: Renewal quantity of replacement product already present should be equal to 10.')
        self.assertEqual(lines_renew[1][1], self.product3.id, 'sales_subscription: First product should be TestProduct')
        self.assertEqual(lines_renew[1][2], 10,
                         'sales_subscription: Renewal quantity of new replacement product should be equal to 3.')
    ...
    def test_18_log_change_template(self):
        ...
        subscription = self.env['sales.subscription'].create({
    ...
    def test_19_fiscal_position(self):
        ...
        self.env['sales.subscription.line'].create({'analytic_account_id': self.subscription.id,
    ...
    def test_prevents_assigning_not_owned_payment_tokens_to_subscriptions(self):
        malicious_user_subscription = self.env['sales.subscription'].create({
