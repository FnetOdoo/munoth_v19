...
class SaleSubscriptionWizard(models.TransientModel):
    _name = "sales.subscription.renew.wizard"
    _description = "Subscription Renew wizard"

    def _default_subscription(self):
        return self.env['sales.subscription'].browse(self._context.get('active_id'))

    subscription_id = fields.Many2one('sales.subscription', string="Subscription", required=True,
    ...
    def _default_products(self):
        ...
        subscription_id = self.env['sales.subscription'].browse(self._context.get('active_id'))
        sub_default_products = subscription_id.archived_product_ids
        res_ids = []
        Options = self.env['sales.subscription.renew.wizard.keep_option']
    ...
    kept_archived_product_ids = fields.One2many('sales.subscription.renew.wizard.keep_option',
                                                'wizard_id',
                                                default=_default_products, string="Archived products",
                                                help="If checked, the product will be reused in the renewal order")
    replacement_line_ids = fields.One2many('sales.subscription.renew.wizard.replace_option',
    ...
    def create_renewal_order(self):
        ...
        return self.subscription_id._prepare_renewal_order(discard_product_ids=discard_ids,
                                                          new_lines_ids=self.replacement_line_ids)


class SaleSubscriptionRenewWizardKeepOption(models.TransientModel):
    _name = "sales.subscription.renew.wizard.keep_option"
    ...
    wizard_id = fields.Many2one('sales.subscription.renew.wizard', required=True, ondelete="cascade")


class SaleSubscriptionRenewWizardReplaceOption(models.TransientModel):
    _name = "sales.subscription.renew.wizard.replace_option"
    _description = "Subscription Renew Lines Wizard"
    ...
    def _compute_product_attributes(self):
        for option in self:
            if option.product_id:
                ...
                if not option.uom_id or option.product_id.uom_id.relative_uom_id.id != option.uom_id.relative_uom_id.id:
