...
class SaleSubscriptionWizard(models.TransientModel):
    _name = 'sales.subscription.wizard'
    ...
    subscription_id = fields.Many2one('sales.subscription', string="Subscription", required=True, default=_default_subscription, ondelete="cascade")
    option_lines = fields.One2many('sales.subscription.wizard.option', 'wizard_id', string="Options")
    ...
    def create_sale_order(self):
        ...
        order.message_post(body=(_("This upsell order has been created from the subscription ") + " <a href=# data-oe-model=sales.subscription data-oe-id=%d>%s</a>" % (self.subscription_id.id, self.subscription_id.display_name)))
...
class SaleSubscriptionWizardOption(models.TransientModel):
    _name = "sales.subscription.wizard.option"
    _description = 'Subscription Upsell Lines Wizard'

    name = fields.Char(string="Description")
    wizard_id = fields.Many2one('sales.subscription.wizard', required=True, ondelete="cascade")
    product_id = fields.Many2one('product.product', required=True, domain="[('recurring_invoice', '=', True)]", ondelete="cascade")
    uom_id = fields.Many2one('uom.uom', string="Unit of Measure", required=True, ondelete="cascade", domain="[('relative_uom_id', '=', product_uom_category_id)]")
